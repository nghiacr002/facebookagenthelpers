...
class Server {
	...
	private function action($client, $action) {
		...
		if($action == "exit" || $action == "quit") {
			...
			\Facebook\Bot::socketBroadCastMessages($client,$action);		
		}
	}
	...
	public function broadCast2Clients($txt, $force_client = null)
	{
		echo "try to broacast 2 all clients ".$force_client."---------";
		if(count($this->clients))
		{
			foreach($this->clients as $key => $client)
			{
				echo  "---".$client->getId();
				if($force_client && $force_client == $client->getId())
				{
					$this->send($client,$txt);
				}
